...
{
    static const TerrainGrid grid = sampleTerrain(islandThree(), 16.0);  // coarse, for speed
    return grid;
}
...
Selection selectFromValley(double pitchDeg)
{
    ...
    static const TerrainLod lod(grid, islandThree(), 200.0);
    const HabitatGeometry&  geometry = islandThree();
    Camera                  camera;
    camera.position =
    ...
    return {.patches = lod.select(camera.position, frustum),
            .layout  = grid.layout,
            .eye     = camera.position};
}
